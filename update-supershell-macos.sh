#!/usr/bin/env bash
###############################################################################
#  ⚡ Super Shell — Updater (macOS)
#
#  Pulls the latest configs and deploys them. Deliberately does NOT touch
#  system state: no brew install/upgrade, no chsh, no font install, no git
#  --global config. Those belong to install-supershell-macos.sh, which you run
#  once per machine (or again when new packages land).
#
#  Run: ./update-supershell-macos.sh [--no-pull] [--dry-run] [--force]
###############################################################################
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            cat << 'USAGE'
Usage: ./update-supershell-macos.sh [--no-pull] [--dry-run] [--force]
...
Never touches: packages, default shell, fonts, git --global config
USAGE
            exit 0
            ;;
        *)
...
# not package names, so this stays honest regardless of brew/manual installs.
section "Tool check"
...
OPTIONAL=(dust duf procs sd xh doggo yazi lazygit delta lazydocker \
          atuin zellij navi tldr gh glab claude trash)
...
if [ ${#missing_required[@]} -eq 0 ] && [ ${#missing_optional[@]} -eq 0 ]; then
    ...
    info "Install them with: ./install-supershell-macos.sh"
fi
